...
def get_s3_filenames(bucket_name, prefix=''):
    ...
    with open("s3_filenames_livevox_calls.csv", 'w', newline= '') as output:
...
def get_database_filenames():
    ...
    try:
        ...
        cursor.execute("SELECT recording_filename FROM livevox_metadata")
        ...
        with open("livevox_calls_metadata_filenames.csv", 'w', newline= '') as output:
            wr = csv.writer(output, dialect='excel')
...
if __name__ == "__main__":
    ...
    with open("in_meta_not_s3_livevox_calls.csv", 'w', newline= '') as output:
        wr = csv.writer(output, dialect='excel')
    ...
    with open("in_s3_not_meta_livevox_calls.csv", 'w', newline= '') as output:
        wr = csv.writer(output, dialect='excel')
    ...
    print ('Number of files in listed in metadata: ' + str(metadata_count))
    print ('Number of files in the S3 bucket: ' + str(s3_count))
